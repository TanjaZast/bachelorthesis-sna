...
\begin{figure}[htb!]
    ...
    \label{fig:Bridge}
\end{figure}
Hierbei erkennen wir gut, dass die drei Subgraphen durch \textit{Brücken} miteinander verbunden sind. Neben den Brücken erkennen wir auch in der Abbildung \ref{fig:Bridge} die Cliquen (1,2,7,8), (3,4,6,5) und (9,10,11). Zu beachten ist, dass es sich bei beispielsweise (1,7,8) oder (4,5,6) um keine Cliquen handelt. Betrachten wir nun Brücken und Cliquen im Kontext zu Zentralitäten erhalten wir folgende Werte, nachdem wir die Berechnungen mit den oben eingeführten Formeln für die Abbildung \ref{fig:Bridge} durchführen. Eine BErechnung für \ref{fig:Clique} ist nicht nötig, da wir in dem anderen Graphen ebenfalls Cliquen und zusätzlich Brücken enthalten haben:
\begin{table}[h!]
\centering
\footnotesize
...
Direkt fällt auf, dass die Werte spaltenweise sehr ähnlich zueinander sind. Bei der \textit{Gradzentralität} sind die Knoten \textbf{3} und \textbf{1} mit einem Wert von \textbf{0.5} und \textbf{0.4} am höchsten. Interessant, denn dabei handelt es sich um die Knoten, die unsere \textit{Brücke} bilden. Betrachten wir nun im folgenden ausschließlich die Knoten \textbf{3} und \textbf{1} fällt auch weiter auf, dass diese Knoten bei der \textit{Nähe-}, \textit{Zwischen-} und \textit{Eigenvektor-Zentralität} ebenfalls am höchsten sind. Das heißt, wir können vermuten, dass Graphen, die Cliquen enthalten haben, relativ ähnliche \textit{Zentralitätswerte} aufweisen beziehungsweise die Varianzen geringer sind. Aber vor allem erwähnenswert ist, dass wir in der Tabelle \ref{table:TableCliqueBridg} lediglich bei den Knoten, welche die \textit{Brücke} bilden, Werte ungleich \textit{Null} besitzen. Dies sollten wir für den weiteren Teil der Arbeit auf jeden Fall in Erinnerung behalten. 
