...
#------------------------------
elist <- c()
ht <- read.csv("tanjalein__insta.csv", sep = ",", stringsAsFactors = FALSE, header = T, fileEncoding = "UTF-8")
...
write.csv(elist_m, "edgelist_tanjalein__insta.csv", fileEncoding = "UTF-8", row.names=F)

#---------------
#Load Edge List
#---------------
imp_matrix <- as.matrix(read.csv("edgelist_tanjalein__insta.csv", sep = ";"))
