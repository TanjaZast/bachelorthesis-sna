...
\If{\textit{matrix} an der Stelle [i][k] größer als \textit{prob} ist}
...
\EndFor
\EndFor
\For {\textit{i} liegt in der \textit{matrix}}
...
\State \text{füge random Matrix in graphs ein} 
...
\For{\textit{i} zwischen \textit{0} und der Länge von \textit{grpahs}}
...
\For{\textit{j und k} zwischen \textit{0} und \textit{graphs}}
